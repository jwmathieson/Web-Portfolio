...
const galleryInfo = [
    ...
    { src: "images/ImageGallery/swordRender.jpg", alt:"An image of a sword", name: "3D Model: Sword", description: "This is a sword I modeled for a set of weapons in a game. The design of the sword's handle,guard, and blade would then be used for everyone other weapon in the set.", type: "image", workType:"3DModel" },
    { src: "images/ImageGallery/magicSwordRender.jpg", alt:"An image of a sword with engravings on the guard and wrappings on the blade.", name: "3D Model: 'Frostmourne'", description: "This is a sword that I modeled for a quest reward in a game. In terms of design this sword is more detailed then any other weapon I have made. Its name 'Frostmourne' was created by another developer. The general idea of the sword is a weapon with ice magic.", type: "image", workType:"3DModel" },
    { src: "images/ImageGallery/spearRender.jpg", alt:"An image of a spear", name: "3D Model: Spear", description: "This is a spear I modeled for a set of weapons in a game. The blade of the spear is bit unique I'd say compared to other bladed weapons I modeled in the past. That design choice makes the spear feel a bit unique in my opinion.", type: "image", workType:"3DModel" },
    { src: "images/ImageGallery/daggerRender.jpg", alt:"An image of a dagger", name: "3D Model: Dagger", description: "This is a dagger I modeled for a set of weapons in a game. Its blade differs from other blades I have made as it curves.", type: "image", workType:"3DModel" },
    { src: "images/ImageGallery/hammerRender.jpg", alt:"An image of a large two-handed hammer", name: "3D Model: Hammer", description: "This is a hammer I modeled for a set of weapons in a game. This model Is more detailed then other weapons in the set. The hammer part of the model has some small details with little bolts that go around it.", type: "image", workType:"3DModel" },
    { src: "images/ImageGallery/theBasiliskRender.jpg", alt:"An image of a large snake with a crystal mane", name: "3D Model: The 'Basilisk'", description: "This model of a large snake is for a enemy in a game. It has a crystal mane and its head is covered in stone like armor. The model also has a rig attached to it so it can be animated.", type: "image", workType:"3DModel" },
    { src: "images/ImageGallery/gameVideo.mp4", name: "Game Programming: First Person Game", description: "This small game project was developed in the engine Godot. I only spend a week on it but I did make great progress at the time. The game was going to be a dungeon exploration game where the player could equip different gear. Each dungeon would be randomly generated and would be selectable from a world map. This project is now discontinued but was a great learning experience for me.", type: "video", workType:"Programming" },
    { src: "images/ImageGallery/pokVideo.mp4", name: "Game Programming: 'Pok'", description: "This was a small project that was being developed in the engine Godot. It was a 3D platformer where you would play as 'Pok', a strange pyramid shaped creature. It was going to be a recreation of one of my old projects from 2022. The game is discontinued and served as a learning experience with the engine.", type: "video", workType:"Programming" },
    { src: "images/ImageGallery/firstEnemy.mp4", name: "Game Programming: Making a Enemy", description: "This was the first enemy I ever coded for a game. The game in the video was a project from 2022 and it was my first game I was developing with programming knowledge. The engine used to develop this game was Roblox Studio. It was the project that helped me learn to program. The game is discontinued now due to burnout.", type: "video", workType:"Programming" },
    { src: "images/ImageGallery/turnBasedCombatGame.mp4", name: "Game Programming: 'Art Journey'", description: "This was my first actual game that people played. It was a turn based combat game where the player would use art supplies to attack. There was multiple different types of abilities and each ability did its own thing. There was multiplayer and players could fight together in battle. Sadly I discontinued this project right after I began recoding it. This is one of my proudest pieces of work that I have ever developed.", type: "video", workType:"Programming" },
...
function updateImageGallery(type, position) {
    ...
    for (i in positionSlotTbl) {
        ...
        if (positional_element.getAttribute("name") == currentGalleryIndex) {
            ...
            positional_element.style.opacity = ".8";

        } else {

            positional_element.style.opacity = ".45";
    ...
    for (i in galleryInfo) {
        ...
        switch (info["workType"]) {
            case "3DModel":
                ...
                newPositionalSlot.style.backgroundColor = "var(--imageGallery-3DModelDisplay,white)";

                break;
        
            case "Programming":

                newPositionalSlot.style.backgroundColor = "var(--imageGallery-ProgrammingDisplay,white)";
        ...
        if (i == 0) {
            ...
            newPositionalSlot.style.opacity = ".8";
